...
Topraksız tarım (coco coir substrat + damlama sulama) yöntemiyle çilek yetiştiriciliği için ESP32-S3 tabanlı otomasyon sistemi. **Sulama** tamamen otomatiktir (büyüme evresine göre zamanlama ve hacim); **iklim** (sıcaklık/nem) için fiziksel bir fan/ısıtıcı/nemlendirici **yok**, bu yüzden sistem iklimi sürekli izler ve hedef dışına çıkınca SMS ile haber verir — düzeltmeyi sen yaparsın (pencere/kapı, taşınabilir fan vb.). Pompa arızaları (tıkanma, kuru çalışma, su seviyesi düşüklüğü) da SMS ile bildirilir ve pompa otomatik kilitlenir. Mac üzerinde çalışan web panelinden canlı/geçmiş veri takibi ve manuel röle kontrolü yapılır.

## Proje Hakkında

- **Yetiştirme yöntemi:** Coco coir substratlı, damlama sulamalı topraksız tarım. Gün-nötr çeşit varsayımıyla sabit fotoperiyot uygulanır (ışık rölesi otomatik).
- **Otomasyonun kapsamı — ne otomatik, ne değil:**
  - ✅ **Sulama** — substrat nemi ve büyüme evresine göre tamamen otomatik (zamanlama + hacim).
  - ✅ **Işık** — fotoperiyota göre otomatik açma/kapama (gerçek röle çıkışı var).
  - ⚠️ **İklim (sıcaklık/nem)** — **otomatik düzeltme yok** (fan/ısıtıcı/nemlendirici donanımı yok); sadece izlenir, hedef dışına çıkınca SMS atılır. 4 kanallı röle kartının 2 kanalı (eski "fan"/"iklim" çıkışları) şu an boşta — ileride bir fan/ısıtıcı bağlanırsa kullanılabilir, kod tarafında yer hazır.
  - ✅ **Pompa güvenliği** — aşırı/düşük akım, su seviyesi, zaman aşımı: otomatik tespit + pompayı kapatma + SMS.
- **Bitki için ne anlama geliyor:** Çilek köküne her zaman doğru zamanda doğru miktarda su/besin gider (susuz kalmaz, fazla sulanıp kök çürümez), ışık döngüsü tutarlı kalır, pompa arızası anında durdurulup donanım korunur. Sıcaklık/nem konforu ise senin elinle/havalandırmanla sağlanır — sistem sana ne zaman, neden müdahale etmen gerektiğini söyler.
...
- **Uzaktan kontrol:** Web panelinden pompa/ışık röleleri ve 2 boş röle kanalı manuel olarak açılıp kapatılabilir, arızalar temizlenebilir.
...
[Sensörler] -> [ESP32-S3 firmware] -> WiFi/HTTP -> [Mac: Node.js web sunucusu] -> [SQLite] -> [Tarayıcı dashboard]
                      |
                      v
                 [Röleler: pompa (otomatik), ışık (otomatik), 2x boş/manuel]
                      |
                      v
...
- `firmware/CilekOtomasyon/` — ESP32-S3 N16R8 Arduino sketch'i:
  ...
  - `control.h/.cpp` — sulama, ışık, pompa güvenliği, iklim izleme (VPD + SMS uyarı) ve röle mantığı
...
| Röle - pompa | 15 | aktif LOW, otomatik sulama |
| Röle - ışık | 17 | aktif LOW, otomatik fotoperiyot |
| Röle 2 (boşta) | 16 | aktif LOW, fiziksel cihaz yok, sadece manuel/ileride kullanım |
| Röle 3 (boşta) | 18 | aktif LOW, fiziksel cihaz yok, sadece manuel/ileride kullanım |
...
Fiziksel fan/ısıtıcı/nemlendirici bulunmadığı için sistem sıcaklık/nem/kök bölgesi sıcaklığını **sadece izler**; hedef aralığın dışına çıkınca bir kere SMS gönderir, aralığa dönünce otomatik temizlenir (manuel "arıza temizle" gerekmez — kilitlenen bir donanım yok, sadece bilgi amaçlı).

- **Sıcaklık hedefi:** Gündüz 20–24°C, gece 13–18°C.
...
- **Spare röle kanalları:** Eski "fan" ve "iklim" röle çıkışları (pin 16, 18) donanım bağlanmadığı için otomatik sürülmüyor; web panelinden manuel açılıp kapatılabilir (test veya ileride bağlanacak bir cihaz için).
...
- `POST /api/control` — gövde: `{ "clearFaults": true }`, `{ "stage": 0-3 }`, veya `{ "relay": "pump"|"light"|"relay2"|"relay3", "on": true|false }` (`relay2`/`relay3` = boşta kanallar, fiziksel cihaz yok)
...
- **Röle kontrolü:** pompa, ışık ve 2 boş röle kanalını (fan/ısıtıcı bağlanırsa kullanılacak) tek tıkla aç/kapat.
- **Uyarılar paneli:** aktif arıza/iklim uyarısı varsa kırmızı, yoksa "Aktif uyarı yok"; "Arızaları Temizle" butonu.
- **24 saatlik geçmiş grafikler:** Sıcaklık/Nem/VPD, Sulama/Drenaj/Drenaj Oranı, Işık, Güç/Enerji — Chart.js ile, dakikada bir yenilenir.
